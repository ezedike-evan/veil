...
const SYSTEM_PROMPT = (walletAddress, feePayerAddress) => `\
...
export async function runAgent(userMessage, walletAddress, agentKeypair, conversationHistory = [], feePayerAddress) {
        ...
        switch (name) {
            ...
            case 'get_transfer_history': {
                ...
                const [wraithResult, horizonResult] = await Promise.allSettled([
                    fetchWithPayment(`${process.env.WRAITH_URL}/transfers/address/${input.address}?direction=${input.direction}&limit=${limit}`),
